...
export default function JobCard({ job, onDelete }) {
  ...
  return (
    <div
      className={`glass rounded-2xl hover:shadow-lg hover:shadow-brand-500/10 transition-all duration-300 group animate-fade-in flex flex-col h-full border border-surface-700/30 border-l-4 ${getBorderAccentColor(job.matchScore)} hover:scale-[1.01]`}
    >
      {/* Header */}
      <div className="px-6 pt-6 pb-4">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          {/* Match Score */}
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border shrink-0 whitespace-nowrap text-xs font-semibold ${getMatchScoreBadgeColor(job.matchScore)}`}
          >
        ...
        {job.reasoning && (
          <div className="bg-surface-900/30 rounded-lg border border-surface-700/20 overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between gap-3 px-4 py-3 hover:bg-surface-900/50 transition-colors"
            >
              <div className="flex items-center gap-2">
              ...
              <ChevronDown
                className={`h-4 w-4 text-surface-500 transition-transform ${showReasoning ? 'rotate-180' : ''}`}
              />
            </button>
            {showReasoning && (
              <div className="px-4 pb-3 pt-0 border-t border-surface-700/20">
                <p className="text-xs text-surface-300 leading-relaxed">{job.reasoning}</p>
              </div>
            )}
          </div>
        )}
        ...
        {job.description && (
          <div className="bg-surface-900/30 rounded-lg border border-surface-700/20 overflow-hidden">
            ...
            >
              ...
              <ChevronDown
                className={`h-4 w-4 text-surface-500 transition-transform ${showDescription ? 'rotate-180' : ''}`}
              />
            </button>
            {showDescription && (
              <div className="px-4 pb-3 pt-0 border-t border-surface-700/20 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-brand-500/30 scrollbar-track-surface-900/20">
                <p className="text-xs text-surface-400 leading-relaxed">{job.description}</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="mt-auto border-t border-surface-700/30 flex items-center gap-3 p-4 bg-surface-950/40">
        ...
        <button
          ...
          disabled={deleting}
          className="ml-auto flex items-center gap-2 px-3 py-2 text-xs font-semibold text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/10 hover:border-red-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-brand-400 border border-brand-500/40 rounded-lg hover:bg-brand-500/10 hover:border-brand-500/60 transition-all disabled:text-surface-600 disabled:border-surface-700/30 disabled:cursor-not-allowed"
